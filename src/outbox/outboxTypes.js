...
import type { Message, Narrow, Reaction, ApiErrorCode } from '../api/apiTypes';
...
/** Information about an API call failure. */
type ApiFailureInfo = $ReadOnly<{|
  /** HTTP failure code. */
  httpStatus: number,
  /** Zulip API failure code (e.g., `BAD_REQUEST`). */
  apiCode: ApiErrorCode,
  /** Hopefully-human-readable error message provided by the server. */
  text: string,
|}>;

/**
 * Failure due to reported client error (4xx return code from API).
 *
 * These are considered program errors, and are reported to Sentry.
 */
export type OutboxFailureClient = $ReadOnly<{|
  type: 'terminal',
...
/**
 * Failure due to presumably-repeated error: the message is now too old to
 * reasonably retry sending.
 *
 * Unlike most other "terminal" failures, failure due to message age is not
 * considered a program error, and is not reported to Sentry.
 */
export type OutboxFailureAge = $ReadOnly<{|
  type: 'terminal',
  subtype: 'age',
...
/**
 * Failure due to generic non-API error.
 *
 * These are considered program errors, and are reported to Sentry.
 */
export type OutboxFailureOther = $ReadOnly<{|
  type: 'terminal',
  subtype: 'misc',
  /** Human-readable. Probably, but not necessarily, from a thrown `Error`. */
  message: string,
  // extras: JSONable, // ?
|}>;

/**
 * Status of a message that has neither been successfully sent nor abandoned.
 *
 * This is the default status of an outbox message on creation. If a nonfatal
 * error occurs while sending, it's retained in the `failure` field.
 */
export type OutboxStatusEnqueued = $ReadOnly<{|
  type: 'transient',
  subtype: 'enqueued',
...
/**
 * Status of an outbox message that has been sent, but has not yet been received
 * from the Zulip server. Redundant with `isSent: true`. (See the documentation
 * for `Outbox`, below.)
 *
 * Sent messages _can_ decay due to age, but this is considered a Sentry-worthy
 * event: the lifetime of a message in the "sent" state should ordinarily be
 * measured in seconds.
 */
export type OutboxStatusSent = $ReadOnly<{|
  type: 'transient',
  subtype: 'sent',
|}>;

type OutboxStatusTerminal = OutboxFailureClient | OutboxFailureAge | OutboxFailureOther;
type OutboxStatusTransient = OutboxStatusEnqueued | OutboxStatusSent;

export type OutboxStatus = OutboxStatusTerminal | OutboxStatusTransient;
